...
func (_q *UserRoleQuery) QueryAssigner() *UserQuery {
	query := (&UserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *UserRoleQuery) WithAssigner(opts ...func(*UserQuery)) *UserRoleQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserRoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserRole, error) {
	...
	if query := _q.withAssigner; query != nil {
		if err := _q.loadAssigner(ctx, query, nodes, nil,
			func(n *UserRole, e *User) { n.Edges.Assigner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserRoleQuery) loadAssigner(ctx context.Context, query *UserQuery, nodes []*UserRole, init func(*UserRole), assign func(*UserRole, *User)) error {
	ids := make([]uint, 0, len(nodes))
	nodeids := make(map[uint][]*UserRole)
	for i := range nodes {
