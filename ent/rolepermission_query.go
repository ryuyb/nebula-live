...
func (_q *RolePermissionQuery) QueryAssigner() *UserQuery {
...
func (_q *RolePermissionQuery) WithAssigner(opts ...func(*UserQuery)) *RolePermissionQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RolePermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*RolePermission, error) {
	...
	if query := _q.withAssigner; query != nil {
		if err := _q.loadAssigner(ctx, query, nodes, nil,
			func(n *RolePermission, e *User) { n.Edges.Assigner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RolePermissionQuery) loadAssigner(ctx context.Context, query *UserQuery, nodes []*RolePermission, init func(*RolePermission), assign func(*RolePermission, *User)) error {
	ids := make([]uint, 0, len(nodes))
	nodeids := make(map[uint][]*RolePermission)
	for i := range nodes {
...
func (_q *RolePermissionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withAssigner != nil {
			_spec.Node.AddColumnOnce(rolepermission.FieldAssignedBy)
		}
	}
